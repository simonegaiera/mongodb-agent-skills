...
impact: HIGH
tags: claude, sonnet, opus, prompting, agentic, thinking, tool-use
---

# Claude Prompting Best Practices

Distilled from the [Claude Prompting Guide](../CLAUDE%20Prompting%20Guide.md). Apply these when building agentic systems and chatbots powered by Claude.

## Core Principles

### Be Clear and Direct

Claude responds best to explicit, specific instructions. Don't rely on the model to infer intent.
...
# GOOD — specific
You are a customer support agent for Acme Corp. Answer questions about our product catalog,
order status, and return policies. If the user asks about something outside these topics,
politely redirect them. Always check the knowledge base before answering.
```

### Use XML Tags for Structure

XML tags help Claude parse complex prompts. Wrap each content type in descriptive tags:

```text
<system_context>
You are a MongoDB-powered chatbot for an e-commerce platform.
</system_context>

<tools_available>
- search_products: Search the product catalog
- check_order_status: Look up an order by ID
- create_return: Initiate a return process
</tools_available>

<response_guidelines>
- Be concise and helpful
- Always confirm before taking actions that modify data
- Quote relevant product details from search results
</response_guidelines>
```

### Provide Context for Motivation

Explain *why* a behavior is important — Claude generalizes from explanations:
...
# GOOD
Never return more than 5 results because the chat interface has limited space and
users find long lists overwhelming. Summarize if there are more matches.
```
...
For agentic coding and tool-using agents, instruct Claude to act rather than suggest:
...
Gate irreversible actions behind confirmation:

```xml
<safety_guidelines>
Consider the reversibility and potential impact of your actions. Take local, reversible
actions freely (reading data, running queries), but for destructive or shared-system
actions (deleting collections, modifying production data), ask the user before proceeding.
</safety_guidelines>
```

### Parallel Tool Calling

Claude excels at parallel tool execution. Enable it explicitly for maximum throughput:

```xml
<use_parallel_tool_calls>
If you intend to call multiple tools and there are no dependencies between the calls,
make all independent calls in parallel. For example, when reading 3 collections, run
3 queries in parallel. Never use placeholders or guess missing parameters.
</use_parallel_tool_calls>
```
...
Claude 4.6 uses adaptive thinking — it decides when and how much to reason. Guide it:

```text
After receiving tool results, carefully reflect on their quality and determine optimal
next steps before proceeding. Use your thinking to plan and iterate based on new
...
answer quality — typically for problems requiring multi-step reasoning. When in doubt,
respond directly.
```
...
```python
# Adaptive thinking (Claude 4.6 — recommended for agents)
...
# Extended thinking with budget (Claude Sonnet 4.6)
client.messages.create(
...
Never speculate about data you have not queried. If the user references a collection
or document, query it before answering. Give grounded, hallucination-free answers.
</investigate_before_answering>
```
...
Include 3–5 examples wrapped in `<example>` tags to steer format and tone:

```xml
<examples>
  <example>
    <user>What's the status of order #12345?</user>
    <assistant>Let me check that for you.
    [calls check_order_status tool]
    Order #12345 is currently "shipped" — it left our warehouse on March 15 and
    is expected to arrive by March 19. Tracking number: ABC123.</assistant>
  </example>
</examples>
```
...
For agents that run across multiple context windows:

1. **Save state to MongoDB** — Use `progress.json` or a MongoDB collection to persist task state
2. **Write tests early** — Create verification scripts before starting work
3. **Use git for checkpointing** — Commit progress incrementally
4. **Don't stop early** — Prompt: "Do not stop tasks early due to token budget concerns. Save progress before context refresh."

Reference: See full guide at `CLAUDE Prompting Guide.md` | https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview
